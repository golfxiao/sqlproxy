...
func (pq *ParsedQuery) GenerateQueryForArgs(args []interface{}) ([]byte, error) {
	bindArgs := make(map[string]interface{}, len(args))
	for i, arg := range args {
		bindArgs[fmt.Sprintf("v%d", i+1)] = arg
	}
	bindVars, err := sqltypes.BuildBindVariables(bindArgs)
	if err != nil {
		return nil, err
	}
...
func (pq *ParsedQuery) ParamsNum() int {
	return len(pq.bindLocations)
}
